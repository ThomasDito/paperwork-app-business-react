...
import { useChangeStatusUserMutation, useLazyGetUsersQuery } from "@/redux/api/superadmin/manage-user/user-api";
...
export default function UserIndex() {
  ...
  // RTK Query
  const [getUsers, { data: users, isLoading: getUsersIsLoading, isFetching: getUsersIsFetching, isError: getUsersIsError }] = useLazyGetUsersQuery();

  const [changeStatusUser] = useChangeStatusUserMutation();

  // States
  const tableIsLoading = getUsersIsLoading || getUsersIsFetching || getUsersIsError;
  ...
  return (
    <>
      <div className="flex flex-col items-center justify-between pb-5 space-y-6 border-0 md:space-y-0 md:flex-row">
        <h3 className="text-2xl font-semibold tracking-tight scroll-m-20">Daftar Pengguna</h3>
      ...
      <div className="border rounded-md shadow-sm bg-card">
        <Table className="whitespace-nowrap">
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="h-auto p-5 text-center">Jumlah Organisasi</TableHead>
              <TableHead className="h-auto p-5">Aktif</TableHead>
              <TableHead className="h-auto p-5 whitespace-nowrap">Tanggal Bergabung</TableHead>
                      ...
                      <div className="flex items-center space-x-4">
                        <Link to={`/user/${user.id}`}>
                          <Avatar className="w-12 h-12">
                            <AvatarImage src={user.user_avatar || ""} alt={user.user_fullname} />
                            <AvatarFallback className="font-medium uppercase text-muted-foreground/50">{user.user_fullname.substring(0, 2)}</AvatarFallback>
                          </Avatar>
                        </Link>
                        <div className="flex flex-col flex-1 space-y-1">
                          <Link to={`/user/${user.id}`} className="text-base font-medium hover:underline">
                            {user.user_fullname}
                          </Link>
                          <div className="text-sm text-muted-foreground">{USER_LEVEL_LABEL[user.user_level]}</div>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className="p-5">{user.user_email}</TableCell>
                    <TableCell className="p-5">{user.user_phone ?? "-"}</TableCell>
                    <TableCell className="p-5 text-center">{user._count.user_organizations || 0}</TableCell>
                    <TableCell className="p-5">
                      <Switch defaultChecked={user.user_status === "active"} onCheckedChange={() => doChangeStatus(user.id)} />
                    </TableCell>
                    <TableCell className="p-5">{moment(user.created_at).format("DD MMMM YYYY")}</TableCell>
                    <TableCell className="p-5 text-center">
                      <Button variant="ghost" className="relative" onClick={() => navigate(`/user/${user.id}`)}>
        ...
        <div className="flex flex-col items-center justify-between px-5 py-3 space-y-4 border-t md:flex-row md:space-y-0">
          <PaginationLimit limit={limit} handleChange={(value) => setLimit(value)} pagination={users?.pagination} />
          {users?.pagination && <PaginationButton pagination={users.pagination} handleChange={(page) => setPage(page)} />}
        </div>
      </div>
    </>
  );
