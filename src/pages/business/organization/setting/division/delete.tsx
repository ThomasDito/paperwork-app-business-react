...
import { useBusinessDivisionDeleteMutation } from "@/redux/api/business/division-api";
...
export default function DivisionDelete() {
  // Hooks
  ...
  // RTK Query
  const [deleteDivision, { isLoading }] = useBusinessDivisionDeleteMutation();
  ...
  const doDelete = async () => {
    ...
    await deleteDivision(id)
      .unwrap()
      .then((response) => {
        toastSuccess(response?.message || "Divisi berhasil dihapus");
        closeModal();
      })
    ...
    <Dialog open={true} onOpenChange={() => closeModal()}>
      <DialogContent>
        <DialogHeader>
          ...
          <p className="pt-5">Apakah anda yakin untuk menghapus divisi ini ?</p>
        </DialogHeader>
