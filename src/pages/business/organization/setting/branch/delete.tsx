...
import { useBusinessBranchDeleteMutation } from "@/redux/api/business/branch-api";
...
export default function BranchDelete() {
  ...
  // RTK Query
  const [deleteBranch, { isLoading }] = useBusinessBranchDeleteMutation();
  ...
  const doDelete = async () => {
    ...
    await deleteBranch(id)
      .unwrap()
      .then((response) => {
        toastSuccess(response?.message || "Cabang berhasil dihapus");
        closeModal();
      })
    ...
    <Dialog open={true} onOpenChange={() => closeModal()}>
      <DialogContent>
        <DialogHeader>
          ...
          <p className="pt-5">Apakah anda yakin untuk menghapus cabang ini ?</p>
        </DialogHeader>
