  ...
  return (
    <Sheet>
      <div className="flex flex-row flex-1 min-h-screen">
        <Sidebar />
        <main className={cn("flex-1 w-full", collapsed ? "lg:ml-[80px]" : "lg:ml-[var(--auth-sidebar-width)]")}>
          <Header className={cn(!collapsed && "md:px-8")} />
          <Navbar />
          <section className={cn("container flex-1 w-full py-12 mx-auto", !collapsed && "md:px-8")}>
            <MobileSidebar />
            <Outlet />
          </section>
        </main>
